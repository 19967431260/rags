...
IM UIKit 提供一套包含 UI 的 IM 接入方式。在 UIKit 中我们对业务场景使用到的 NIM SDK 的接口封装成业务数据能力层，将复杂的数据关联调用精简成一次接口调用，减少上层业务的复杂性。本文概要性介绍业务数据能力层的接口。
...
account | 获取登录账号 | 获取登录账号接口
getUserInfo | 获取登录用户信息 | 获取登录用户信息接口
...
会话列表模块分为`ConversationKit-ui` 和 `ConversationKit`两层设计，其中`ConversationKit-ui`提供会话列表界面相关的 UI 和交互逻辑，`ConversationKit`是对会话相关业务能力的封装，向 UI 层提供业务数据聚合接口和 IM SDK 接口的封装。相较于 IM SDK 的接口，`ConversationKit` 的接口拥有更好的业务聚合性，更方便您在 UI 层的展示和操作。以会话列表查询接口为例，IM SDK 的会话列表查询接口，调用成功后仅返回会话信息；而`ConversationKit`的会话列表查询接口，调用成功后的返回结果不仅包含会话信息，也包含该会话对应的用户信息或群组信息。 


### 获取 UIKit 会话列表

`ConversationKit`模块下的`ConversationRepo`类，提供操作/配置会话列表的相关方法和属性。其中调用会话列表查询接口的示例代码如下：
...
方法/属性 | <div style="width:100px">功能描述</div> | 参数
:---- | :-------------- | :---------
getSessionList | 获取会话列表 | 参数如下<br><div><ul><li>`limit`：获取会话列表数量</li><li>`callback`：查询结果回调返回`List<ConversationInfo>`</li></ul></div>
addStickTop | 会话置顶 | 参数如下<br><div><ul><li>`sessionId`：会话ID，个人会话为对方账号ID (即 accid)，群组会话则为群ID</li><li>`typeEnum`：会话类型，详见[`SessionTypeEnum`](https://doc.yunxin.163.com/docs/interface/messaging/android/doxygen/Latest/zh/enumcom_1_1netease_1_1nimlib_1_1sdk_1_1msg_1_1constant_1_1_session_type_enum.html)</li><li>`ext`：扩展字段，String类型，最大512字符</li><li>`callback`：结果回调，回调参数为`StickTopSessionInfo`</li></ul></div>
removeStickTop | 取消会话置顶| 参数如下<br><div><ul><li>`sessionId`：会话ID，个人会话为对方账号ID (即 accid)，群组会话则为群ID</li><li>`typeEnum`：会话类型，详见[`SessionTypeEnum`](https://doc.yunxin.163.com/docs/interface/messaging/android/doxygen/Latest/zh/enumcom_1_1netease_1_1nimlib_1_1sdk_1_1msg_1_1constant_1_1_session_type_enum.html)</li><li>`ext`：扩展字段，String类型，最大512字符</li><li>`callback`：结果回调</li></ul></div>
deleteSession | 删除会话| 参数如下<br><div><ul><li>`sessionId`：会话ID，个人会话为对方账号ID (即 accid)，群组会话则为群ID</li><li>`typeEnum`：会话类型，详见[`SessionTypeEnum`](https://doc.yunxin.163.com/docs/interface/messaging/android/doxygen/Latest/zh/enumcom_1_1netease_1_1nimlib_1_1sdk_1_1msg_1_1constant_1_1_session_type_enum.html)</li><li>`deleteTypeEnum`：删除类型`DeleteTypeEnum`，可设置删除本地消息或者远端消息</li><li>`sendAck`：废弃</li><li>`callback`：结果回调</li></ul></div>
getMsgUnreadCount | 获取消息未读数 | 无
registerSessionChangedObserver | 注册会话变化监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`EventObserver<List<ConversationInfo>>`</li></ul></div>
unregisterSessionChangedObserver | 取消注册会话变化监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`EventObserver<List<ConversationInfo>>`</li></ul></div>
registerSessionDeleteObserver | 注册会话删除监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`EventObserver<ConversationInfo>`</li></ul></div>
unregisterSessionDeleteObserver | 取消注册会话删除监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`EventObserver<ConversationInfo>`</li></ul></div>
registerUserInfoObserver | 注册用户信息变化监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`UserInfoObserver`</li></ul></div>
unregisterUserInfoObserver | 取消注册用户信息变化监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`UserInfoObserver`</li></ul></div>
registerFriendObserver | 注册好友信息变化监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`FriendObserver`</li></ul></div>
unregisterFriendObserver | 取消注册好友信息变化监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`FriendObserver`</li></ul></div>
registerTeamRemoveObserver | 注册群组移除变化监听器  | 参数如下<br><div><ul><li>`observer`：监听器，类型为`Observer<Team>`</li></ul></div>
unregisterTeamRemoveObserver | 取消注册群组移除变化监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`Observer<Team>`</li></ul></div>
registerTeamUpdateObserver | 注册群组信息变化监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`Observer<List<Team>>`</li></ul></div>
unregisterTeamUpdateObserver | 取消注册群组信息变化监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`Observer<List<Team>>`</li></ul></div>
registerFriendMuteObserver | 注册好友消息不提醒变化监听器 |参数如下<br><div><ul><li>`observer`：监听器，类型为`Observer<MuteListChangedNotify>`</li></ul></div>
unregisterFriendMuteObserver | 取消注册好友消息不提醒变化监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`Observer<MuteListChangedNotify>`</li></ul></div>
registerAddStickTopObserver | 注册会话置顶变化监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`Observer<StickTopSessionInfo>`</li></ul></div>
unregisterAddStickTopObserver | 取消注册会话置顶变化监听器 |参数如下<br><div><ul><li>`observer`：监听器，类型为`Observer<StickTopSessionInfo>`</li></ul></div>
registerRemoveStickTopObserver | 注册置顶移除变化监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`Observer<StickTopSessionInfo>`</li></ul></div>
unregisterRemoveStickTopObserver | 取消注册置顶移除变化监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`Observer<StickTopSessionInfo>`</li></ul></div>
registerUpdateStickTopObserver | 注册置顶会话变化监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`Observer<StickTopSessionInfo>`</li></ul></div>
unregisterUpdateStickTopObserver | 取消注册置顶会话变化监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`Observer<StickTopSessionInfo>`</li></ul></div>
registerSyncStickTopObserver | 注册置顶会话同步监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`Observer<List<StickTopSessionInfo>>`</li></ul></div>
unregisterSyncStickTopObserver | 取消注册置顶会话同步监听器 | 参数如下<br><div><ul><li>`observer`：监听器，类型为`Observer<List<StickTopSessionInfo>>`</li></ul></div>
...
getFriendInfo | 从本地获取用户资料| 参数如下<br><div><ul><li>`accId`：用户id</li></ul></div>
