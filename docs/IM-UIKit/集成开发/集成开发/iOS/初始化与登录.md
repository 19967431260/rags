...
table th:nth-of-type(3) {width: 60%;}
</style>

根据本文操作前，请确保您已经完成了以下设置：

- 已完成 [组件导入](https://doc.yunxin.163.com/messaging-uikit/guide/TA5MzA4ODU?platform=iOS)。
...
为保障通信安全，如果您在调试环境中的使用的是 [网易云信控制台](https://app.yunxin.163.com/global/home) 生成的 IM 账号（测试用），请确保在后续的正式生产环境中，将其替换为通过 [IM 服务端 API](https://doc.yunxin.163.com/messaging2/server-apis/TQyNjgyMzc?platform=server) 生成的正式 IM 账号。