...
`ContactUIConfig` 是整个通讯录的配置文件，其中 UI 实例化对象类型为 `ContactUIConfig`。
...
`showTitleBarRightIcon` | Bool | 是否展示标题栏的最右侧图标，具体自定义示例请参考下文的 [隐藏标题栏搜索按钮和添加好友按钮](#隐藏标题栏搜索按钮和添加好友按钮)。
`showTitleBarRight2Icon` | Bool | 是否展示标题栏的次最右侧图标，具体自定义示例请参考下文的 [隐藏标题栏搜索按钮和添加好友按钮](#隐藏标题栏搜索按钮和添加好友按钮)。
`titleBarRightRes` | UIImage | 标题栏的最右侧图标。
`titleBarRight2Res` | UIImage | 标题栏的次最右侧图标。
`titleBarRightClick` | () -> Void | 标题栏最右侧按钮单击事件。
`titleBarRight2Click` | () -> Void | 标题栏次最右侧按钮单击事件。
`title` | String | 标题栏文案。
`titleColor` | UIColor | 标题栏字体颜色，具体自定义示例请参考下文的 [自定义标题栏](#自定义标题栏navigationview)。
`showHeader` | Bool | 是否在通讯录界面显示头部模块，具体自定义示例请参考下文的 [隐藏头部模块](#隐藏头部模块)。
...
`contactProperties` | ContactProperties | 通讯录好友列表中 UI 个性化定制属性设置，包括好友昵称的字体、好友头像、索引字体、颜色等。
  ...
  `itemTitleSize` | UIFont | 通讯录好友标题大小，具体自定义示例请参考下文的 [修改通讯录好友标题字体颜色和大小](#修改通讯录好友标题字体颜色和大小)。
  `itemTitleColor` | UIColor | 通讯录好友标题颜色，具体自定义示例请参考下文的 [修改通讯录好友标题字体颜色和大小](#修改通讯录好友标题字体颜色和大小)。
  `divideLineColor` | UIColor | 通讯录间隔线颜色，具体自定义示例请参考下文的 [修改通讯录间隔线颜色](#修改通讯录间隔线颜色)。
  `indexTitleColor` | UIColor | 检索标题字体颜色。
`friendItemClick` | (ContactInfo, IndexPath) -> Void | 好友列表的单击事件。
`headerItemClick` | (ContactInfo, IndexPath) -> Void | 头部模块的单击事件。
...
<img style="width:60%;border: 2px solid #f2f2f2;" src="https://yx-web-nosdn.netease.im/common/38653405f59787c81b84d99b59621362/通讯录布局新.png" />
...
`navigationView` | TabNavigationView | 通讯录列表界面标题视图，可在子类直接获取视图进行样式修改，具体自定义示例请参考下文的 [自定义标题栏](#自定义标题栏navigationview)。
`bodyTopView` | UIView | 好友列表上方的小块视图，可在子类直接获取视图进行样式修改。用于在通讯录列表和顶部标题中间增加新的 UI 元素，具体自定义示例请参考下文的 [通讯录列表顶部添加扩展视图](#通讯录列表顶部添加扩展视图)。
...
`tableView` | UITableView | 通讯录界面的列表，可自定义单元格，实现数据的自定义展示，具体自定义示例请参考下文的 [自定义单元格](#自定义单元格contacttableviewcell)。
`emptyView` | NEEmptyDataView | 通讯录列表界面空数据提示视图，可在子类直接获取视图进行样式修改。
`bodyBottomView` | UIView | 好友列表底部的小块视图，可在子类直接获取视图进行样式修改，用于在好友列表底部增加新的 UI 元素，具体自定义示例请参考下文的 [通讯录列表底部添加扩展视图](#通讯录列表底部添加扩展视图)。

## 通过配置项自定义 UI 示例

::: note notice
通讯录的个性化 UI 样式需要在通讯录界面加载之前配置，例如您可以在 `IMKitClient` 的 `login` 方法中设置个性化定制属性。
...
- 示例代码
  ...
  ```Swift
  // 修改通讯录好友标题大小
  ContactUIConfig.shared.ui.contactProperties.itemTitleSize = 28

  /// 修改通讯录好友标题颜色
  ContactUIConfig.shared.ui.contactProperties.itemTitleColor = UIColor.red
  ```
  :::
  ::: tab Objective-C
  ```objc
  ...
  ContactUIConfig.shared.ui.contactProperties.itemTitleSize = 28;
  //修改通讯录好友标题颜色
  ContactUIConfig.shared.ui.contactProperties.itemTitleColor = [UIColor redColor];
  ```
  :::
  ::::::
...
- 效果对比
     ...
     默认 | 自定义后 |
     ---- | ---- |
     <img alt="通讯录默认.png" src="https://yx-web-nosdn.netease.im/common/53437622dee7267b02739c4529e5e99f/通讯录默认.png" style="width:60%;"> | <img alt="通讯录字体颜色.png" src="https://yx-web-nosdn.netease.im/common/a4b541222078e248934626cd2d092154/通讯录字体颜色.png" style="width:60%;">
...
- 示例代码
  ...
  ```Swift
  //隐藏搜索按钮
  ContactUIConfig.shared.ui.showTitleBarRight2Icon = false
  //隐藏添加好友按钮
  ContactUIConfig.shared.ui.showTitleBarRightIcon = false
  ```
  :::
  ::: tab Objective-C
  ```objc
  //隐藏搜索按钮
  ContactUIConfig.shared.ui.showTitleBarRight2Icon = NO;
  //隐藏添加好友按钮
  ContactUIConfig.shared.ui.showTitleBarRightIcon = NO;
  ```
  :::
  ::::::
...
- 效果对比
     ...
     默认 | 自定义后 |
     ---- | ---- |
     <img alt="通讯录默认.png" src="https://yx-web-nosdn.netease.im/common/53437622dee7267b02739c4529e5e99f/通讯录默认.png" style="width:60%;"> | <img alt="通讯录隐藏右侧按钮.png" src="https://yx-web-nosdn.netease.im/common/f5717ea90a2dc7a13cee5d3fe2f474ab/通讯录隐藏右侧按钮.png" style="width:60%;">
...
- 示例代码
  ...
  ::: tab Swift
  ```Swift
  ContactUIConfig.shared.ui.contactProperties.divideLineColor = UIColor.red
  ```
  :::
  ::: tab Objective-C
  ```objc
  ContactUIConfig.shared.ui.contactProperties.divideLineColor = [UIColor redColor];
  ```
  :::
  ::::::
...
- 效果对比
     ...
     默认 | 自定义后 |
     ---- | ---- |
     <img alt="通讯录默认.png" src="https://yx-web-nosdn.netease.im/common/53437622dee7267b02739c4529e5e99f/通讯录默认.png" style="width:60%;"> | <img alt="通讯录间隔线颜色.png" src="https://yx-web-nosdn.netease.im/common/e599887c6f70d76c8aea31dd5ed81175/通讯录间隔线颜色.png" style="width:60%;">
...
- 示例代码
  ...
  ::: tab Swift
  ```Swift
  ContactUIConfig.shared.ui.showHeader = false
  ```
  :::
  ::: tab Objective-C
  ```objc
  ContactUIConfig.shared.ui.showHeader = NO;
  ```
  :::
  ::::::
...
- 效果对比
     ...
     默认 | 自定义后 |
     ---- | ---- |
     <img style="width:60%;border: 2px solid #f2f2f2;" src="https://yx-web-nosdn.netease.im/common/4a8cd6a0b571962d5bad42230448e35e/原图.PNG" /> | <img style="width:60%;border: 2px solid #f2f2f2;" src="https://yx-web-nosdn.netease.im/common/d09dfb526e22a4a9e906df8538c2f377/通讯录隐藏头部.PNG" />
...
2. 继承 ContactsViewController 类，实现自定义表格。
    ...
    public class CustomContactsViewController: ContactsViewController, ContactsViewControllerDelegate {
        ...
        delegate = self
        // 自定义 cell, [ContactCellType.ContactCutom.rawValue: 需要注册的自定义 UI 类型]
    ...
    `onDataLoaded` 方法调用时，表明父类已完成数据的加载，数据存放在 `viewModel.contacts`（类型为 `[ContactSection]`）列表中，数据模型定义为：
...
| 默认 | 自定义后 |
| ---- | ---- |
| <img alt="通讯录没小标默认.png" src="https://yx-web-nosdn.netease.im/common/50ab683403218a536403ee7c4def47df/通讯录没小标默认.png" style="width:60%;"> | <img alt="通讯录头像小标.png" src="https://yx-web-nosdn.netease.im/common/edf533c19c7f00bdd1c5362a13615785/通讯录头像小标.png" style="width:60%;">

### 自定义标题栏（navigationView）

自 V9.6.1 起，自定义导航栏需要使用 `customNavigationView` 来实现，V9.6.5 更名为 `navigationView`。
...
- 效果对比
     ...
     默认 | 自定义后 |
     ---- | ---- |
     <img alt="消息详情页默认.png" src="https://yx-web-nosdn.netease.im/common/1f92c687026734891bbb897355bda14e/消息详情页默认.png" style="width:60%;"> | <img alt="消息详情自定义右侧图标.png" src="https://yx-web-nosdn.netease.im/common/746ede517ebaec0dd3a02935a1a3b13c/消息详情自定义右侧图标.png" style="width:60%;">


    ::: details 单击查看 V9.6.1 之前版本实现自定义标题栏按钮的 Swift 示例代码。
    ```Swift
    public class CustomContactsViewController: ContactsViewController, ContactsViewControllerDelegate {
        override public func viewDidLoad() {
        super.viewDidLoad()
        ...
        if ContactUIConfig.shared.ui.hiddenRightBtns {
            return
        } else {
            if ContactUIConfig.shared.ui.hiddenSearchBtn {
...
- 效果对比
     ...
     默认 | 自定义后 |
     ---- | ---- |
     <img alt="通讯录默认.png" src="https://yx-web-nosdn.netease.im/common/53437622dee7267b02739c4529e5e99f/通讯录默认.png" style="width:60%;"> | <img alt="通讯录自定义右侧按钮.png" src="https://yx-web-nosdn.netease.im/common/035203374eab1d3a2ea3cf235d836ff4/通讯录自定义右侧按钮.png" style="width:60%;">
...
- 效果对比
     ...
     默认 | 自定义后 |
     ---- | ---- 
     <img alt="通讯录默认.png" src="https://yx-web-nosdn.netease.im/common/53437622dee7267b02739c4529e5e99f/通讯录默认.png" style="width:60%;"> | <img alt="通讯录提示消息.png" src="https://yx-web-nosdn.netease.im/common/9f52995f097e8d9a90c4a1338fa28e92/通讯录提示消息.png" style="width:60%;">
...
- 示例代码（Swift）
    ...
    public class CustomContactsViewController: ContactsViewController, ContactsViewControllerDelegate {
        ...
        // 底部 bodyBottomView 中添加自定义 view（需要设置 bodyBottomView 的高度）
...
- 效果对比
    ...
    默认 | 自定义后 |
    ---- | ---- |
    <img style="width:60%;border: 2px solid #f2f2f2;" src="https://yx-web-nosdn.netease.im/common/4a8cd6a0b571962d5bad42230448e35e/原图.PNG" /> | <img style="width:60%;border: 2px solid #f2f2f2;" src="https://yx-web-nosdn.netease.im/common/6b02c859c30725b183246c8e677740f7/通讯录底层扩展视图.png" />