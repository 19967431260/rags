网易云信 IM UIKit 是基于 [网易云信 IM SDK（NIM SDK）](https://doc.yunxin.163.com/messaging2/concept?platform=client) 开发的一款 [即时通讯 UI 组件库](https://doc.yunxin.163.com/messaging-uikit/concept/TI3NTgyNDA?platform=client)，功能涵盖了聊天、会话、群管理等，您可以基于源码搭建您的即时通讯业务逻辑。IM Electron UIKit（`nim-uikit-electron`）[开源代码](https://github.com/netease-kit/nim-uikit-electron) 托管在 GitHub 仓库。
...
| 依赖 | 说明 |
| --- | --- |
| `node-nim` | [网易云信 NIM SDK](https://doc.yunxin.163.com/messaging2/concept?platform=client)，提供了一整套即时通讯基础能力。 |
...
您可以参考 [集成 UIKit（Vue 3）](https://doc.yunxin.163.com/messaging2/guide/DM1NjMzNjA?platform=electron) 将 IM UIKit 源码集成至您的项目。