...
根据本文操作前，请确保您已经完成 [IM UIKit 初始化](https://doc.yunxin.163.com/messaging-uikit/guide/DM1NjMzNjA?platform=electron#第三步集成并初始化组件)。

## 发送自定义消息

以 **发送一条评价消息** 为例，发送时，可通过 `createCustomMessage` 创建一条自定义消息，调用 `sendMessage` 进行发送。

![Electron评价消息收发.gif](https://yx-web-nosdn.netease.im/common/50bc6f07ee3f535cb0a77a241c835dc2/Electron评价消息收发.gif)
...
同样以 **接收评价消息** 为例，接收方可根据消息体上的 `props.msg.attachment` 上的自定义字段，判断当前消息是否为评价消息，从而自行实现任意的 UI 展示。
