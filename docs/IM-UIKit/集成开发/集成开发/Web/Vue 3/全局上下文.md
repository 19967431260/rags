...
在使用 IM UIKit `store` 前，请确保您已完成 IM UIKit 的初始化。详情请参考 [初始化](https://doc.yunxin.163.com/messaging-uikit/guide/jMzMzQ0MDk?platform=web#第三步集成并初始化组件)。
...
使用 `autorun` 来监听 `store` 数据的变化，从而改变 UI。具体使用说明请参考 [Mobx 官方文档](https://mobx.js.org/README.html "Mobx 官网文档")。
