...
2. 如果未登录，跳转至登录页。
    ...
    在登录页，完成业务服务器登录用户账号后，调用 app.vue 中的 [`init`](#init) 方法（可将此方法封装成一个全局函数，便于调用），并传入业务服务器返回的账号 ID（`account`）和密钥（`token`）。
...
3. 退出登录。
    ...
    调用 `store.logout()` 以及 `nim.V2NIMLoginService.logout()` 方法。可参考 GitHub 中的 [`logout`](https://github.com/netease-kit/nim-uikit-web/blob/main/vue3-demo/src/views/chat/components/setting.vue) 方法。
