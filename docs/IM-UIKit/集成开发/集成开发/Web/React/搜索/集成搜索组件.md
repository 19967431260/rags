...
如果默认搜索界面无法满足您的业务需求，请根据自定义参数进行自定义，或者基于 NIM SDK 自行实现搜索。搜素组件（`SearchContainer`）提供的自定义参数，请参考 [定制搜索界面](https://doc.yunxin.163.com/messaging-uikit/guide/TQ2MDEyMzE?platform=web#%E8%87%AA%E5%AE%9A%E4%B9%89%E5%8F%82%E6%95%B0)。