...
<img alt="image-netease" src="https://yx-web-nosdn.netease.im/common/9949c5baff35cd31a334d2338f3fcfc3/%E8%81%8A%E5%A4%A91.gif" style="width:80%;border: 1px solid #BFBFBF;">

## 前提条件

根据本文操作前，请确保您已经完成了以下设置：

- [导入组件](https://doc.yunxin.163.com/messaging-uikit/guide/TYyNjcyOTY?platform=web)。
- [初始化组件](https://doc.yunxin.163.com/messaging-uikit/guide/Tg0NjMzMzg?platform=web)。
...
2. 在需要展示聊天页面时，动态展示 `ChatContainer` 组件，调用 `store` 中的 `selectConversation` 或 `insertConversationActive`，以及插入会话，传入对应参数。
    ...
    <div>
        ...
        <div :style="{ height: '700px' }" ref="chat" />
    </div>
    </template>
    <script>
    import { ChatContainer } from '@xkit-yx/im-kit-ui';
