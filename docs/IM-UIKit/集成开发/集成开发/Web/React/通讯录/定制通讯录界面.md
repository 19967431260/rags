如果默认通讯录界面（`ContactListContainer`）无法满足您的业务需求，您可通过本节介绍的参数进行界面自定义。

## 自定义参数

如果默认界面无法满足您的业务需求，请根据以下的自定义参数进行自定义。

通讯录组件（`ContactListContainer`）提供的自定义参数如下：

### 关系导航

<div style="width:180px">参数</div> | 类型 | 说明 |
:---- | :---- | :--- |
`renderCustomContact` | `((contactType: ContactType) => Element)` | 自定义渲染通讯录内容，具体示例参考 [自定义渲染通讯录内容](#自定义通讯录内容)。 |
...
`renderFriendListHeader` | `(() => Element)` | 自定义渲染好友列表头部内容，具体示例参考 [自定义好友列表头部内容](#自定义好友列表头部内容)。 |
...
`renderGroupListHeader` | `(() => Element)` | 自定义渲染[群组列表头部内容](#群组列表为空时的内容)。 |
...
- 示例代码解读
    ...
    上述示例中的 contactType 可获取到五种类型，分别为 'blackList'（黑名单） | 'groupList'（我的群组）| 'friendList'（我的好友） | 'msgList'（消息中心）｜'aiList'， 可根据不同 contactType 自定义渲染逻辑，若 renderCustomContact 返回为 null，则使用组件默认渲染逻辑。
...
- 示例代码
    ...
    <ContactInfoContainer
        ...
        afterAcceptApplyFriend={(account) => {
            const textMsg = nim.V2NIMMessageCreator.createTextMessage('我已经同意了你的申请，现在开始聊天吧~')
