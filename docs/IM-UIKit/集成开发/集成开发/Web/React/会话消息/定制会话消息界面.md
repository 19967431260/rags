如果默认会话界面模块（`ChatContainer`）无法满足您的业务需求，您可通过聊天组件的自定义参数进行界面自定义。
...
<span id="renderTeamInputPlaceHolder">`renderTeamInputPlaceHolder`</span>
...
<span id="renderTeamMemberItem">`renderTeamMemberItem`</span>
...
<span id="msg:-V2NIMMessageForUI">`msg`</span>
...
<span id="MsgOperMenuItem[]">`MsgOperMenuItem[]`</span>
...
<span id="ChatSettingActionItem[]">`ChatSettingActionItem[]`</span>

```JavaScript
interface ChatSettingActionItem{
  // 唯一 key
  action: ChatAction
...
`chat-kit` 模块提供的自定义参数如下：

| 参数 | 参数类型 | 说明 |
|---|---|---|
| `actions` | [`Action[]`](#Action[])  | 消息发送按钮组配置，不传使用默认的配置  <br> 具体的示例参考下文 [自定义消息发送按钮](#自定义消息发送按钮)。 |
| `renderP2pInputPlaceHolder` |  (conversation: V2NIMConversation) => JSX.Element | 自定义渲染单聊聊天输入框 `placeholder`。具体的示例参考下文 [自定义输入框 placeholder](#自定义输入框-placeholder)。 |
| `renderTeamInputPlaceHolder` |  [`renderTeamInputPlaceHolder`](#renderTeamInputPlaceHolder) | 自定义渲染群组聊天输入框 `placeholder`。具体的示例参考下文 [自定义输入框 placeholder](#自定义输入框-placeholder)。 |
| `renderTeamMemberItem` |  [`renderTeamMemberItem`](#renderTeamMemberItem) |  自定义渲染 [群组成员 `item`](#群成员-item)。 |
| `afterTransferTeam` |  (teamId: string) => Promise<void> | 转让群主后的回调。 |
| `renderP2pCustomMessage` | ((options: RenderP2pCustomMessageOptions) => Element \| null) | 渲染单聊中的整条自定义消息，头像、昵称、消息体均需要自行实现。 |
| `renderTeamCustomMessage` | ((options: RenderTeamCustomMessageOptions) => Element \| null) | 渲染群聊中的自定义消息，头像、昵称、消息体均需要自行实现。 |
| `renderEmpty` | (() => Element) | 自定义渲染未选中任何会话时的内容。具体示例参考下文 [自定义未选中会话时的内容](#自定义未选中会话时的内容)。 |
| `renderHeader` |  (conversation: V2NIMConversation) => JSX.Element | 自定义渲染会话的 header 样式。具体示例参考下文 [自定义会话 header 渲染](#自定义会话-header-渲染)。 |
| `prefix` | string | 样式前缀，默认值：`chat`。 |
| `commonPrefix` | string | 公共样式前缀，默认值：`common`。 |
| `renderMessageOuterContent` |   [`msg`](#msg:-V2NIMMessageForUI) | 自定义渲染会话消息，气泡样式也需要自定义。具体示例参考下文 [自定义渲染消息体](#自定义渲染消息体)。 |
| `renderMessageInnerContent` |  [`msg`](#msg:-V2NIMMessageForUI) | 自定义渲染会话消息，不需要自定义气泡样式。具体示例参考下文 [自定义渲染消息体](#自定义渲染消息体)。 |
| `msgOperMenu` |  [`MsgOperMenuItem[]`](#MsgOperMenuItem[]) | 自定义渲染消息右键菜单，包括菜单项是否显示（show）、菜单项名称（label、key、icon）、自定义点击事件（onClick）。具体示例参考下文 [自定义消息右键操作列表](#自定义消息右键操作列表)。 |
| `p2pSettingActions` | [`ChatSettingActionItem[]`](#ChatSettingActionItem[]) | 自定义单聊消息页面右侧的设置栏按钮。具体示例参考下文 [自定义消息页面右侧设置栏按钮](#自定义消息页面右侧设置栏按钮)。 |
| `teamSettingActions` | [`ChatSettingActionItem[]`](#ChatSettingActionItem[]) | 自定义群聊消息页面右侧的设置栏按钮。具体示例参考下文 [自定义消息页面右侧设置栏按钮](#自定义消息页面右侧设置栏按钮)。 |
|  `msgRecallTime` |  number |  消息可撤回的最大时间，单位毫秒，默认 2 * 60 * 1000, 最大支持7天内的消息可撤回。 |
|  `renderMessageAvatar` |  [`msg`](#msg:-V2NIMMessageForUI) |  自定义渲染消息头像。 |
|  `renderMessageName` |   [`msg`](#msg:-V2NIMMessageForUI) |  自定义渲染消息昵称。 |
|  `strangerTipVisible` | boolean | 是否展示陌生人的提示信息。 |
|  `scrollIntoMode` | `nearest` | 上拉加载消息滚动模式。组件在上拉加载消息时，默认会自动滚动到最新的消息，当组件位于可滚动的页面中时，可能会造成滚动异常，设置`nearest` 即可。 | 
|  `maxUploadFileSize` | number | 上传的最大文件大小，单位 MB，默认 100MB。 | 
| `onMessageItemAvatarClick` | `onMessageItemAvatarClick?: (user: V2NIMUser) => void` | 聊天列表头像点击事件。|
| `enableSendVideo` | boolean |  是否允许发送视频，可选。默认为 true，允许发送视频。 | 
...
- 效果比对
    ...
    :---- | :----
    <img alt="Web 默认消息体.png" src="https://yx-web-nosdn.netease.im/common/67a741ea9bd43966c4f12c1438101ea9/Web默认消息体.png" style="width:80%;border: 0px solid #BFBFBF;"> | <img alt="渲染消息体out.png" src="https://yx-web-nosdn.netease.im/common/ed4cdc6d118b1faa493534e4df8063d4/渲染消息体out.png" style="width:80%;border: 0px solid #BFBFBF;"> | <img alt="渲染消息体in.png" src="https://yx-web-nosdn.netease.im/common/79085b7b4305af7e0f378a697e42f2a2/渲染消息体in.png" style="width:80%;border: 0px solid #BFBFBF;">
        ...
        ...
            ...
            mounted() {
                ...
                actions: [
                    ...
                    render: () => {
                        return compile(
                        `<div onClick={context.click} style={{margin: '0 10px', fontSize: '14px'}}>
                            <i className="iconfont icon-logout" />
