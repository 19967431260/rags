...
IM UIKit 将 `localeConfig` 中的字符串本地化为目标语言。您可以使用另一种语言显示文本，覆盖 `localeConfig` 中的默认值。

具体完整 `LocaleConfig`，请参考 GitHub 示例（[中文](https://github.com/netease-kit/nim-uikit-web/blob/release/9.8.0/react/src/components/IMApp/locales/zh.ts) | [英文](https://github.com/netease-kit/nim-uikit-web/blob/release/9.8.0/react/src/components/IMApp/locales/en.ts)）。

:::note note
`LocaleConfig` 中内置表情（emoji）使用固定的取值，无需根据语系修改。
:::

以中文语系为例，`LocaleConfig` 设置内容如下：
...
// 改为您自身项目工程中 localeConfig 的实际路径即可
import zh from './components/locales/zh.js';
