...
在登录成功之后，您需要根据网易云信控制台配置的 AI 数字人进行筛选，并确定负责 AI 翻译或者 AI 划词的数字人。然后在接口方法中，将网易云信控制台上配置的 AI 数字人作为入参，如下所示。更多详情，请参考 IM UIKit Demo 中 [`MainActivity`](https://github.com/netease-kit/nim-uikit-android/blob/master/app/src/main/java/com/netease/yunxin/app/im/main/MainActivity.java) 中的代码。

```Java
//设置功能 AI 数字人信息 AIUserManager.setProvider(
...
建议您在 `Application` 或者会话列表页面加载之前配置即可：

```Java
IMKitConfigCenter.setEnableAIUser(false);
```
