...
1. 前往[云信开源代码仓库](https://github.com/netease-kit)，下载 `nim-uikit-android` 源码组件到本地。
...
6. （可选）源码依赖处理冗余文件，减少资源引入。
    ...
    云信 IM UIKit 提供两套风格的 [UI 组件库](https://doc.yunxin.163.com/messaging-uikit/guide/TI3NTgyNDA?platform=android)，可任意选择一种使用。确定后，可移除另一套 UI 组件相关文件，以减小包体积。可删除的文件如下：
    ...
    - 资源文件：在各个模块的 main 目录中，分别有 res-normal 和 res-fun 文件夹。可直接删除您不需要的 UI 版本目录。
        ...
        ```
    :::note notice
    对应 UI 版本的路由注册代码需要单独移除。（以 `chatkit-ui` 为例，`contactkit-ui`、`conversationkit-ui` 、`teamkit-ui` 同理。）
...
导入组件后，需进行[初始化](https://doc.yunxin.163.com/messaging-uikit/guide/jY3MDA4NzY?platform=android)。
...
集成过程中的常见问题的排查说明，请参见[IM UIKit 常见问题排查](https://doc.yunxin.163.com/messaging-uikit/guide/DE2MDc3NTM?platform=android)。