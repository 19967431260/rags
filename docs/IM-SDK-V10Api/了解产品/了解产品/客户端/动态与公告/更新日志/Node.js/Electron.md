本文介绍网易云信即时通讯 IM SDK（简称 NIM SDK）开发版 Electron 端 10.x.x 及以上版本的更新日志。

::: note note
- 有关稳定版 9.x.x 及以下版本，请参考《IM 即时通讯 V9》[Electron 更新日志](https://doc.yunxin.163.com/messaging/concept/TgxNDQ4Nzc?platform=client)。
- 有关 UI 组件 10.x.x 及以上版本的更新日志，请参考 [Electron UIKit 更新日志](https://doc.yunxin.163.com/messaging-uikit/concept/zY3NzM1NDQ?platform=client)。
:::
...
## <span id="10.9.75 - 2026-02-02">10.9.75 (2026-02-02)</span>
...
## <span id="10.9.72 - 2026-01-05">10.9.72 (2026-01-05)</span>
...
## <span id="10.9.53 - 2025-10-11">10.9.53 (2025-10-11)</span>
...
## <span id="10.9.40 - 2025-08-08">10.9.40 (2025-08-08)</span>

**升级说明**

IM Node.js/Electron SDK 自 10.9.40 发布后 **不再自动创建全局的 v2、nim、qchat、chatroom 变量**，开发者升级后需主动创建相关对象，例如：
...
// 创建 API SDK 实例
const nim = new V2NIMClient();
// 初始化 SDK
nim.init({})
// 获取业务服务
nim.getLoginService()?.login('account ID', ' token', {})
// 反初始化 SDK
nim.uninit()
```
:::
...
## <span id="10.9.10 - 2025-06-19">10.9.10 (2025-06-19)</span>
...
`pushMobileOnDesktopOnline` | 新增回调，用于监听当桌面端在线时，移动端是否需要推送。
...
## <span id="10.8.10 - 2025-03-21">10.8.10 (2025-03-21)</span>
...
`enableCloudConversation` | 云端会话的配置开关，默认为 `false`，代表不启用云端会话模式（默认使用本地会话模式）。只有设置为 `true` 后，才能正常使用 [`V2NIMConversationService`](https://doc.yunxin.163.com/messaging2/client-apis/zA4NjQzOTQ?platform=client#接口类) 服务。
`V2NIMMessageSearchExParams` | 本地历史消息检索接口的入参中新增 `messageSubtypes` 字段，表示自定义消息子类型。

## <span id="10.8.0 - 2025-02-18">10.8.0 (2025-02-18)</span>
...
`getMessageList` | 查询消息列表方法的入参新增 `onlyQueryLocal` 字段，表示是否是查询本地消息。
-->

## <span id="10.6.1 - 2024-12-06">10.6.1 (2024-12-06)</span>
...
## <span id="10.6.0 - 2024-11-22">10.6.0 (2024-11-22)</span>
...
`getChatroomQueueService` | 获取聊天室队列服务（除 Web 端）。
...
## <span id="10.5.0 - 2024-10-16">10.5.0 (2024-10-16)</span>
...
- 新增本地登录信息的查询功能。
- 新增 [更新消息](https://doc.yunxin.163.com/messaging2/guide/jg1NjE1NDY?platform=client) 功能，实现消息的二次编辑能力。
- 收藏对象支持唯一并去重。
...
[`V2NIMAddCollectionParams`](https://doc.yunxin.163.com/messaging2/client-apis/DAxNjk0Mzc?platform=client#V2NIMAddCollectionParams) | 新增 `uniqueId`，用于去重。 |
...
## <span id="10.3.1 - 2024-07-23">10.3.1 (2024-07-23)</span>
...
方法/回调/类 | 说明
---- | ----
[`getDndConfig`](https://doc.yunxin.163.com/messaging2/client-apis/TMwMTgwMDA?platform=client#getDndConfig)|新增方法，用于获取推送免打扰配置详情。
...
方法/回调/类 | 说明
---- | ----
[`getAddApplicationUnreadCount`](https://doc-pre.netease.im/messaging2/client-apis/jM1MTQ0NDQ?platform=client#getAddApplicationUnreadCount) | 新增方法，用于查询未读的好友申请数量。
[`setAddApplicationRead`](https://doc-pre.netease.im/messaging2/client-apis/jM1MTQ0NDQ?platform=client#getAddApplicationUnreadCount) | 新增方法，用于将所有未读的好友申请标记为已读。
...
**v10.2.0 版本发布**。平台包括 Android、iOS、macOS/Windows、Web（Web/小程序/uniapp）、Linux、Electron。
