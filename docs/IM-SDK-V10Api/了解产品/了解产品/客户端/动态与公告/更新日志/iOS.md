本文介绍网易云信即时通讯 IM SDK（简称 NIM SDK）开发版 iOS 端 10\.x\.x+ 及以上版本的更新日志。

::: note note
- 有关稳定版 9.x.x 及以下版本，请参考《IM 即时通讯 V9》[iOS 更新日志](https://doc.yunxin.163.com/messaging/concept/jQ3NzQ3NDI?platform=iOS)。
- 有关 UI 组件 10.x.x 及以上版本的更新日志，请参考 [iOS UIKit 更新日志](https://doc.yunxin.163.com/messaging-uikit/concept/jE5MTA5NjA?platform=client)。
:::
...
## <span id="10.9.54 - 2025-10-23">10.9.54 (2025-10-23)</span>

- 升级最近会话表，增加消息子类型（`Subtype`）字段。
- 修复已知问题。
...
## <span id="10.9.51 - 2025-09-18">10.9.51 (2025-09-18)</span>
...
`registerBadgeCountHandler` | 新增方法，用于注册获取 badge 数量的回调函数。
...
`NIMServerSetting.chatRoomLbsList` | 新增字段，表示聊天室 LBS 服务器地址。
`NIMChatroomEnterRequest.enableLbs` | 新增方法，用于启用聊天室 LBS。默认为 false，不启用。
`NIMSDKConfig.enableEnhancedLinkSecurity` | 新增方法，用于启用增强链路安全策略，默认为 YES，启用。
...
## <span id="10.3.2 - 2024-07-19">10.3.2 (2024-07-19)</span>
...
- 优化插入本地消息接口（`insertMessageToLocal`），与其他端对齐参数。
- 优化消息体中的 `serverExtension` 字段的格式，不强制限定 JSON 格式，但建议使用 JSON。
