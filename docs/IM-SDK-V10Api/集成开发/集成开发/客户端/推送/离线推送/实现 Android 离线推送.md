...
<!-- <img style="width:80%" src="https://yx-web-nosdn.netease.im/common/76ff7bea6bf4bfe6bdbab0d4e101c460/Android 离线推送技术原理.png" /> -->
...
- 已 [集成 Android SDK](https://doc.yunxin.163.com/messaging2/guide/DA5NzIyMjc?platform=client)。
...
| 谷歌 FCM | `firebase-bom:33.0.0`<br>具体版本：`firebase-messaging：24.0.0`，`firebase-analytics: 22.0.0` | [集成谷歌推送](https://doc.yunxin.163.com/messaging/guide/zY2Nzc4MjY?platform=android)

## 第二步：集成 NIM SDK

在 [集成 NIM SDK](https://doc.yunxin.163.com/messaging2/guide/DA5NzIyMjc?platform=client) 时，需要添加第三方厂商推送辅助包。
...
- 若手动集成需要将 [下载](https://doc.yunxin.163.com/messaging2/resource?platform=all)的 `nim-push-x.x.x.jar` 文件拷贝到您的项目路径的 `app/libs` 目录下。然后在该 `.jar` 文件上右键添加为依赖。

## 第三步：初始化 NIM SDK

[初始化 NIM SDK](https://doc.yunxin.163.com/messaging2/guide/TY4OTgyMDk?platform=client) 时，完成厂商的推送证书配置，并选择推送渠道。
...
2. 选择推送渠道。
    ...
    - 如果 [`SDKOptions.mixPushConfig.autoSelectPushType`](https://doc.yunxin.163.com/messaging2/references/android/doxygen/Latest/zh/classcom_1_1netease_1_1nimlib_1_1sdk_1_1mixpush_1_1_mix_push_config.html#a1bc6b2fec84b3b732b501dd6c44bc507) 为 false（默认），则 SDK 直接选择服务端推荐的推送渠道。
...
这里以发送文本消息为例，通过调用 `sendMessage` 实现。发送的消息默认需要推送，如需设置推送文案，推送角标，推送文案前缀等，请参考 [配置消息的推送属性](https://doc.yunxin.163.com/messaging2/guide/TY4MzU5MDc?platform=android)。
...
:::note note
- 离线推送支持配置免打扰时间，具体请参考 [设置推送全局免打扰](https://doc.yunxin.163.com/messaging2/guide/DM5ODc1ODg?platform=android)。
- 离线推送支持配置多端推送策略，即支持推送至同一账号的多个客户端，具体请参考 [设置多端推送策略](https://doc.yunxin.163.com/messaging2/guide/jQ3MTc1MjI?platform=android)。
:::

## 第五步：（可选）关闭或开启离线推送

NIM SDK 的第三方推送服务功能默认开启，无需单独调用开启服务接口。若后续不想使用第三方推送服务，可调用 [`enable`](https://doc.yunxin.163.com/messaging2/references/android/doxygen/Latest/zh/interfacecom_1_1netease_1_1nimlib_1_1sdk_1_1mixpush_1_1_mix_push_service.html#a30b0a583ee96874a154124ffa07004cb) 方法进行关闭。
...
- 第三方厂商推送通道存在流控，具体请参考 [厂商推送通道限制说明](https://doc.yunxin.163.com/messaging2/guide/TQyOTE4Nzk?platform=client#%E5%8E%82%E5%95%86%E9%80%9A%E9%81%93%E9%99%90%E6%B5%81)

- 若您想直接使用 SDK 中注册的 Channel 来作为第三方厂商（小米、OPPO）的申请 Channel，可参考 [SDK 中的 Channel 信息](https://doc.yunxin.163.com/messaging2/guide/TQyOTE4Nzk?platform=client#SDK%E9%80%9A%E9%81%93Channel)

- [第三方推送厂商通道错误码参考](https://doc.yunxin.163.com/messaging2/guide/TQyOTE4Nzk?platform=client#%E5%8E%82%E5%95%86%E9%80%9A%E9%81%93%E9%94%99%E8%AF%AF%E7%A0%81%E6%96%87%E6%A1%A3)

- 支持自行填入第三方厂商的推送 Token。

    ::: details 具体实现步骤
    1. 设置 [`MixPushConfig.manualProvidePushToken`](https://doc.yunxin.163.com/messaging2/references/android/doxygen/Latest/zh/classcom_1_1netease_1_1nimlib_1_1sdk_1_1mixpush_1_1_mix_push_config.html#a92522b985cbdabb6a8bbec28d7622802) 为 true，表示支持自行传入第三方推送 token。

        ```Java
        SDKOptions options = new SDKOptions();
        MixPushConfig mixPushConfig = new MixPushConfig();
        // other mixPushConfigs, mixPushConfig.xxx = xxx
        // 是否自行传入推送 token
        mixPushConfig.manualProvidePushToken = true; // true 为打开自行传入第三方推送 token 开关；false 为关闭
        options.mixPushConfig = mixPushConfig;
        // 初始化动作
        // ...
        ```

    2. 调用 [`NIMPushClient.registerManuallyProvidePushTokenCallback`](https://doc.yunxin.163.com/messaging2/references/android/doxygen/Latest/zh/classcom_1_1netease_1_1nimlib_1_1sdk_1_1mixpush_1_1_n_i_m_push_client.html#a35dba45f3f132a4d8e7ed9006c5d786d) 方法，注册用户自主提供的推送 token 回调（`IManualProvidePushTokenCallback`）。

        [`IManualProvidePushTokenCallback`](https://doc.yunxin.163.com/messaging2/references/android/doxygen/Latest/zh/interfacecom_1_1netease_1_1nimlib_1_1sdk_1_1mixpush_1_1_i_manual_provide_push_token_callback.html) 回调中的 [`onToken`](https://doc.yunxin.163.com/messaging2/references/android/doxygen/Latest/zh/interfacecom_1_1netease_1_1nimlib_1_1sdk_1_1mixpush_1_1_i_manual_provide_push_token_callback.html#add023db241bf1ce57d5b03729c93dc11) 方法可以设置当前设备的推荐第三方推送类型（[`suggestedPushType`](https://doc.yunxin.163.com/messaging2/references/android/doxygen/Latest/zh/enumcom_1_1netease_1_1nimlib_1_1sdk_1_1mixpush_1_1model_1_1_mix_push_type_enum.html)）。设置好后，回调会返回 **推送类型 + 对应的第三方推送 token** 的键值对交由云信 NIM SDK 处理。详细示例代码请参考 [DemoManualProvidePushTokenCallback](https://yx-web-nosdn.netease.im/common/bcd9fd39ee6915c4b0d453bc806ffbb5/DemoManualProvidePushTokenCallback.java) 文件。

        示例代码如下：
        
        ```Java
        // 注册自定义推送Token
        NIMPushClient.registerManuallyProvidePushTokenCallback(new DemoManualProvidePushTokenCallback(context));
        ```
    :::
