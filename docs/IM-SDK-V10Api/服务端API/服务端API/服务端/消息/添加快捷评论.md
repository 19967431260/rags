网易云信服务端支持对指定的消息（包括单聊消息、高级群消息和超大群消息）添加快捷评论。
...
```
PATCH https://{endpoint}/im/v2/messages/actions/quick_comment
```
...
参数名称 | 类型 | 是否必选 | 描述
--- | --- | --- | ---
`operator_id` | String | 是 | 进行添加快捷评论操作的用户账号 ID。
