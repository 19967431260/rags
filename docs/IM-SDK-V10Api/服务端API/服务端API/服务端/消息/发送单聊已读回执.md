该接口可以在单聊会话中发送消息已读回执，即在单聊会话中将某消息标记为已读。
...
```
POST https://{endpoint}/im/v2/messages/actions/p2p_read_receipt
```
...
参数名称 | 类型 | 是否必选 | 描述
---- | ---- | ---- | ----
`from_account_id` | String | 是 | 已读回执发送方（即消息接收方）的 IM 账号 ID。
`to_account_id` | String | 是 | 已读回执接收方（即消息发送发）的 IM 账号 ID。
`message_server_id` | Long | 是 | 需要标记为已读的服务端消息 ID。
