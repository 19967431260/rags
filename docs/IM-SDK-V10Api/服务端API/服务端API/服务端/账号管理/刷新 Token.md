<!--keywords: token,Token,刷新,IM 账号,账号管理,IM 账号-->


本接口可以刷新指定 IM 账号的 Token（由网易云信服务器随机生成）。更新后，需要同时更新自身服务器上维护的 Token，此外也要确保客户端再次登录时携带的 Token 是最新的。

## 功能描述

网易云信 IM 服务端支持刷新 Token。

- 不指定 Token，随机刷新。可通过本文介绍的接口对 Token 进行随机刷新。
- 指定 Token 刷新。如果需要更新成指定的 Token，则可以调用 [更新账号属性](https://doc.yunxin.163.com/messaging2/server-apis/TgwNTIyODk?platform=server) 接口，修改为指定的 Token。
...
```
PATCH https://{endpoint}/im/v2/accounts/{account_id}/actions/refresh_token
```
...
参数名称 | 类型 | 是否必选 | 描述
---- | ---- | ---- | ----
`account_id` | String | 是 | 需要刷新 Token 的 IM 账号 ID。
