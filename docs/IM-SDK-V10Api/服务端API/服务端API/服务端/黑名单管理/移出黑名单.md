<!--keywords: 解除静音,静音管理,静音 -->

该接口可以将指定的 IM 账号移出黑名单。移出黑名单后，双方可以互相发送消息。
...
```
DELETE https://{endpoint}/im/v2/block_contacts/{account_id}
```
...
参数名称 | 类型 | 是否必选 | 描述
:---- | :---- | :---- | :----
`account_id` | String | 是 | 发起移出黑名单操作的 IM 账号 ID。

### 查询参数

参数名称 | 类型 | 是否必选 | 描述
:---- | :---- | :---- | :----
`contact_account_id` | String | 是 | 需要被移出黑名单的 IM 账号。
