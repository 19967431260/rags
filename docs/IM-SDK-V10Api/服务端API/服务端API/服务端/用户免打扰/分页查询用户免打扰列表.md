<!--keywords: 获取,查询,静音列表,免打扰列表 -->

该接口可以分页查询指定账号的免打扰用户列表，列表中包含被该账号设置免打扰的用户账号 ID。
...
```
GET https://{endpoint}/im/v2/mute_contacts
```
...
`data` | Object | 返回的 JSON 数据对象，请求失败则返回空对象。 | 是 |
 ...
 `items` | Array of strings | 返回的免打扰账号信息。 | 否 |
  `contact_account_id` | String | 免打扰的 IM 账号。 | 否
