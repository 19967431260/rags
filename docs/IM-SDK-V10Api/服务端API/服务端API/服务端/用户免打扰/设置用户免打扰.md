<!--keywords: 静音,免打扰,消息免打扰 -->

该接口可以将指定的 IM 账号设置为免打扰。

## 功能描述

设置免打扰后，当被设为免打扰的用户发送消息给自己时，不会触发在线消息提醒和离线推送的通知栏。

每个账号可设置免打扰的用户上限为 3000。
...
```
POST https://{endpoint}/im/v2/mute_contacts
```
...
参数名称 | 类型 | 是否必选 | 描述
:---- | :---- | :---- | :----
`account_id` | String | 是 | 发起免打扰操作的 IM 账号 ID。
`contact_account_id` | String | 是 | 需要被设置为免打扰的 IM 账号 ID。
...
| 105429 | 不允许对自己进行免打扰操作 | self mute operation not allowed
| 105435 | 免打扰列表超出上限 | mute list limit exceeded
