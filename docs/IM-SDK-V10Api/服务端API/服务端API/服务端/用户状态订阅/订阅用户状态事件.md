网易云信 IM 服务端支持用户状态事件的订阅功能。该接口用于订阅指定用户的用户状态，包括在线状态，或自定义状态。每个账号最多有效订阅 3000 个账号。
...
```
POST https://{endpoint}/im/v2/subscription/{account_id}
```
...
参数名称 | 类型 | 是否必选 | 描述
---- | ---- | ---- | ----
`account_id` | String | 是 | 订阅者账号 ID。

### 查询参数

参数名称 | 类型 | 是否必选 | 描述
---- | ---- | ---- | ----
`duration` | Long | 是 | 订阅的有效时间，单位为秒。取值范围：60s～2592000s（30 天）

### 请求体参数

参数名称 | 类型 | 是否必选 | 描述
---- | ---- | ---- | ----
`account_ids` | Array of strings | 是 | 发布用户状态事件的用户账号列表。单次最多操作 100 个账号。
...
`data` | Object | 返回的 JSON 数据对象，请求失败则返回空对象。 | 是
 `failed_list` | Array of objects | 订阅失败的用户账号列表。 | 是
  `account_id` | String | 用户账号 ID。 | 否
  `error_code` | Integer | 订阅失败的错误码。 | 否
  `error_msg` | String | 订阅失败的错误信息。 | 否
