...
```
DELETE https://{endpoint}/im/v2/team_members/actions/kick_member
```
...
参数名称 | 类型 | 是否必选 | 描述
---- | ---- | ---- | ----
`operator_id` | String | 是 | 操作者（群主或和管理员）账号 ID。
`team_id` | Long | 是 | 群组 ID，创建群组成功时，网易云信服务器会自动生成并返回。
`team_type` | Integer | 是 | 群组类型。1：高级群。2：超大群。
`kick_account_ids` | Array of strings | 是 | 踢出群的成员列表。<note type=note>一次最多可以踢 10 个群成员。
...
`data` | Object | 返回的 JSON 数据对象，请求失败则返回空对象。 | 是
 `success_list` | Array of strings | 踢除成功的用户账号 ID 列表。 | 是
 `failed_list` | Array of objects | 踢除失败的用户信息列表。 | 否
  `account_id` | String | 踢除失败的用户的 IM 账号 ID。 | 否
