该接口可以将群管理员变更为群组普通成员，包括高级群和超大群。

::: note notice
- 只有群主才能移除群管理员身份。
- 可以批量移除群管理员，但单次操作最多不超过 10 人。
:::
...
```
DELETE https://{endpoint}/im/v2/teams/{team_id}/actions/remove_manager
```
...
`managers` | Array of strings | 是 | 需要移除管理员身份的群成员列表。单个元素为群管理员的账号 ID，一次最多移除 10 个管理员。
