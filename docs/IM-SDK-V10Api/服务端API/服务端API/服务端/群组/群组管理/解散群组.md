该接口可以解散群组，包括高级群和超大群。只有群主才能解散群组。
...
```
DELETE https://{endpoint}/im/v2.1/teams/{team_id}
```
...
`team_type` | Integer | 是 | 解散的群组类型。1：高级群。2：超大群。
...
```BASH
curl -X DELETE \
  'https://api.yunxin.163.com/im/v2.1/teams/123456789?team_type=1&operator_id=test_user&extension=%7B%22reason%22%3A%22%E7%BE%A4%E7%BB%84%E8%A7%A3%E6%95%A3%22%7D' \
  -H 'AppKey: your_app_key' \
  -H 'Nonce: random_string' \
  -H 'CurTime: 1640995200' \
  -H 'CheckSum: checksum_value' \
