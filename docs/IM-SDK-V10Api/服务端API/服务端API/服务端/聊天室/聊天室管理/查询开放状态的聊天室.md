...
```
GET https://{endpoint}/im/v2/chatrooms/actions/opend_chatrooms
```
...
参数名称 | 类型 | 是否必选 | 描述
---- | ---- | ---- | ----
`creator_account_id` | String | 是 | 聊天室创建者账号 ID。
...
`data` | Object | 返回的 JSON 数据对象，请求失败则返回空对象。 | 是
 `items` | Array of strings | 开放的聊天室 ID 列表。 | 否
