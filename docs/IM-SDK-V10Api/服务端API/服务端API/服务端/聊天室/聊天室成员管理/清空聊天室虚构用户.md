...
```
DELETE https://{endpoint}/im/v2/room_members/actions/clear_virtual_members
```
...
`notification_enabled` | Boolean | 否 | 清空聊天室中的虚构用户是否发送通知，默认为 false（不通知）。
