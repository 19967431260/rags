...
该接口用于在聊天室中批量删除指定的虚构用户，一次最多删除 100 个虚构用户。
...
```
DELETE https://{endpoint}/im/v2/room_members/actions/virtual_members
```
...
`account_ids` | Array of strings | 是 | 需要删除的虚构用户账号列表，一次最多删除 100 个虚构用户。
`notification_enabled` | Boolean | 否 | 删除聊天室中的虚构用户是否需要通知，默认为 true（通知）。
...
`data` | Object | 返回的 JSON 数据对象，请求失败则返回空对象。 | 是
 `failed_list` | Array of objects | 删除失败的虚构用户账号列表。 | 否
  `account_id` | String | 虚构用户账号 ID。 | 否
  `error_code` | Integer | 删除失败的错误码。 | 否
  `error_msg` | String | 删除失败的错误信息。 | 否
 `success_list` | Array of strings | 删除成功的账号列表。 | 否
