...
```
PATCH https://{endpoint}/im/v2/room_members/{account_id}/actions/blocked
```
...
参数名称 | 类型 | 是否必选 | 描述
---- | ---- | ---- | ----
`account_id` | String | 是 | 需要拉黑或取消拉黑的成员账号 ID。
...
`operator_account_id` | String | 是 | 操作者账号 ID。只有聊天室创建者或管理员才能拉黑或取消拉黑聊天室成员。
`blocked` | Boolean | 是 | 是否拉黑。true：设置为拉黑状态；false：取消拉黑状态。
