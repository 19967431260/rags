...
```
PATCH https://{endpoint}/im/v2/room_members/{account_id}/actions/temp_chat_banned
```
...
参数名称 | 类型 | 是否必选 | 描述
---- | ---- | ---- | ----
`account_id` | String | 是 | 需要设置临时禁言的成员账号 ID。
...
`operator_account_id` | String | 是 | 操作者账号 ID。只有聊天室创建者或管理员才能临时禁言/取消临时禁言聊天室成员。
