本文介绍网易云信房间组件（NeteaseRoom，简称 NERoom）Android 端的更新日志。有关 NERoom 组件依赖的 `NIMSDK`、`NERtcSDK`、`NEWhiteboard`，请参考 [IM 即时通讯](https://doc.yunxin.163.com/messaging2/concept?platform=client)、[音视频通话](https://doc.yunxin.163.com/nertc/concept?platform=client)、[互动白板](https://doc.yunxin.163.com/whiteboard/concept?platform=web)。
...
- 兼容 `Common` 1.8.0 版本
- 兼容 `CoreKit` 1.9.0 版本
- 兼容 `CoreKit-IM2` 1.9.0 版本
- 兼容 `NEWhiteboard` 3.9.15 版本

## <span id="1.43.1 (2025-11-21">1.43.1 (2025-11-21)</span>
...
类/方法/回调/错误码 | 说明
--- | --- 
`NERoomRtcController.startScreenShare` | 新增方法，用于开启屏幕共享。较 `startScreenShare(Intent, MediaProjection.Callback, NECallback<Unit>)` 接口，入参新增屏幕共享配置 `configuration`（`NERoomVideoSubStreamConfiguration`），可设置屏幕共享的视频参数。

**变更**

`NESeatController` 重命名为 `NERoomSeatController` 类，并保留 `NESeatController` 类向后兼容。
...
- 修改 Http 超时时间。连接超时由 30s 改为 15s。读超时改为 30s。
- `serverUrl` 支持 https 前缀格式补全。
- 将 `NERoomHistoryMessageSearchOption` 从 impl 目录移动至 api 目录对外。
...
类/方法/回调/错误码 | 说明
--- | --- 
`NERoomRtcController.startChannelMediaRelay` | 新增方法，开始跨房间推流转发。该接口替换原接口（`startChannelMediaRelay(NECallback<Unit> callback)`），较原接口入参新增房间 ID（`roomUuid`）。注：原接口废弃。
...
`NERoomKit.switchDomain` | 新增方法，用于切换 SDK 服务器域名。
`NERoomKit.getAvailableDomain` | 新增方法，用于获取 SDK 可用的服务器域名。
...
类/方法/回调/错误码 | 说明
--- | --- 
`NERoomMemberInviteState` | 新增数据结构，表示成员邀请状态，用于标记成员被邀请过程中的状态。
...
- 兼容 `Common` 1.3.3 版本
- 兼容 `CoreKit` 1.4.14 版本
- 兼容 `CoreKit-IM2` 1.1.2 版本
...
类/方法/回调/错误码 | 说明
--- | --- 
`NERoomRtcController.setPlaybackBeforeMixingAudioFrameParameters` | 新增方法，用于设置播放远端混音前的数据格式。
`NERoomRtcController.setCameraTorchOn` | 新增方法，用于设置摄像头是否开启闪光灯。
`NERoomRtcController.setCloudProxy` | 新增方法，用于是否开启云代理服务。
`NERoomRtcChannelProfile.karaoke` | 新增方法，用于指定 K 歌模式。
`NERoomRtcChannelProfile.meeting` | 新增方法，用于指定会议模式。
`NEWhiteboardController.setupWhiteboardCanvas` | 新增方法，用于支持白板初始化配置防盗链开关。
`NERoomEndReason` | 新增 `CLOSE_ON_CONNECTOR_SESSION_TIMEOUT` 错误码类型，表示三方会议加入失败。
...
修复将 `NERoomRtcController.muteMyAudio` 方法的入参 `enableMediaPub` 设置为 true 时仍听不到伴音的问题。
...
## <span id="1.32.2 (2024-10-25)">1.32.2 (2024-10-25)</span>
...
## <span id="1.31.1 (2024-10-18)">1.31.1 (2024-10-18)</span>
...
- 兼容 `NIMSDK` 9.17.2 版本
- 兼容 `NERtcSDK` 5.6.2008-SNAPSHOT 版本
- 兼容 `Common` 1.3.3 版本
- 兼容 `CoreKit` 1.4.11 版本
- 兼容 `CoreKit-IM` 1.4.12 版本
- 兼容 `NEWhiteboard` 3.9.6 版本
...
- 兼容 `Common` 1.3.3 版本
- 兼容 `CoreKit` 1.4.11 版本
- 兼容 `CoreKit-IM` 1.4.11 版本
- 兼容 `NEWhiteboard` 3.9.6 版本

## <span id="1.30.1 (2024-07-26)">1.30.1 (2024-07-26)</span>

**新增特性**

- 支持离线消息推送能力，具体实现流程请参考 [实现离线推送](https://doc.yunxin.163.com/messaging/guide/zc1OTI2MTM?platform=android)。
- 支持音视频房间语音转字幕功能。
...
`NERoomKitOptions.mixPushConfig`|新增属性，用于设置离线消息推送配置。
...
## <span id="1.28.1 (2024-05-14)">1.28.1 (2024-05-14)</span>
...
- 支持在房间中查询历史消息和撤回消息。
- 新增云端录制会议能力，具体请参考 [云端录制](https://doc.yunxin.163.com/neroom/guide/zE4MzI1NzA?platform=android)。
...
## <span id="1.22.2 (2023-11-08)">1.22.2 (2023-11-08)</span>
...
|--------------------------------|-----------------------------------------|----------------|
| 房间禁言      | 允许房间创建者或管理员在房间内将所有成员禁言，同时也禁止所有成员开启音频和视频功能。| [房间禁言](https://doc.yunxin.163.com/neroom/guide/DcxMTQ2Njc?platform=android)
| 黑名单        | 允许房间创建者或管理员将房间内的指定成员添加到黑名单，黑名单中的成员将被移除出房间且无法重新加入房间。 | [黑名单](https://doc.yunxin.163.com/neroom/guide/Dg1MzQ0MTI?platform=android)
| 成员禁言        | 允许房间创建者或管理员设置指定成员禁言或使用音频和视频功能。当成员被禁言时，将无法在房间内发消息，使用音频和视频功能。 | [成员禁言](https://doc.yunxin.163.com/neroom/guide/TI5MjMzOTM?platform=android)
...
| <div style="width:80px">新增特性</div> | <div style="width:150px">特性描述</div> | <div style="width:80px">相关文档</div> |
|--------------------------------|-----------------------------------------|----------------|
| 支持使用 IM 账号登录 NERoom        |  如果您的 App 之前已集成了 IM SDK，已经存在 IM 账号，可以用 `IMKitClient.init` 替换原先的 `NIMClient.init`，再通过 NERoom 提供的 `loginByIM` 接口登录 NERoom。|  [复用 IM 的账号](https://doc.yunxin.163.com/neroom/guide/Dk5NjQ3Mjg?platform=android)

**改进优化**

`onDisconnect` 不再触发 `NERoomListener` 的 `onRtcChannelError` 回调，改为内部处理直接销毁 `roomContext`。
...
|接口名称 | 接口说明 | 
|---- | -------------- | 
|`NERoomRtcController.setAudioProfile`|将 `setLocalAudioProfile` 接口名称修改为 `setAudioProfile`。
...
| <div style="width:80px">新增特性</div> | <div style="width:150px">特性描述</div> | <div style="width:80px">相关文档</div> |
|--------------------------------|-----------------------------------------|----------------|
| 支持多媒体消息                           |  支持发送与接收图片消息和文件消息|  <a href="https://doc.yunxin.163.com/neroom/guide/TM1NjkyNzQ?platform=android" target="_blank">消息发送</a> <br><a href="https://doc.yunxin.163.com/neroom/guide/TE5Mzc1NjU?platform=android" target="_blank">消息接收</a>
...
| <div style="width:80px">新增特性</div> | <div style="width:150px">特性描述</div> | <div style="width:80px">相关文档</div> |
|--------------------------------|-----------------------------------------|----------------|
| 虚拟背景                           | NERoom SDK 通过自动识别用户人像，虚化用户周围的真实环境，或者以指定颜色的图片或自定义图像替代真实背景，从而实现设置虚拟背景。 | [虚拟背景](https://doc.yunxin.163.com/neroom/guide/zE0Njg0MTU?platform=android)           |
| 云信美颜|云信自研的基础美颜和高级美颜功能，支持在音视频通话或互动直播场景中，对人脸进行美肤、美型等美颜调整，或通过画面滤镜改变视频的色调与氛围。|[云信美颜](https://doc.yunxin.163.com/neroom/guide/DIyODE3NzM?platform=android)|
...
| <div style="width:80px">新增特性</div> | <div style="width:150px">特性描述</div> | <div style="width:80px">相关文档</div> |
|--------------------------------|-----------------------------------------|----------------|
| 耳返                           | 耳返即耳机采集监听，在设备上插入耳机或耳麦后，可以从耳机侧听到本设备麦克风采集到的声音，主要用来监听本地采集的音频。耳返音频具备低延时、高音质等特征，可以实时向主播等参与音频互动的成员反馈本端的声音数据，让主播可以实时听到本端的声音。耳返功能一般用于在线 KTV、连麦 PK、演唱会等娱乐场景。 | [耳返](https://doc.commsease.com/neroom/guide/Tk4MjAyMzA?platform=android)           |
| 伴音和音效                     | 支持通过混音功能播放掌声、口哨等短时音效，或者为人声添加背景音乐、伴奏音乐或其他场景效果，并将合成后的声音播放给房间内其他成员。在音视频通话或直播场景中，可以更好的烘托气氛、营造多样化语音环境。      | [伴音和音效 ](https://doc.commsease.com/neroom/guide/TM0MjUzMjQ?platform=android)    |
| 房间属性设置和更新             | 房间属性是附加到当前房间上的一系列 key-value键值对， key为属性唯一名称， value 为当前属性值。 NERoom 允许开发者自定义房间属性，并在房间生命周期内对属性进行添加、更新、删除操作，同时还可以监听属性变更通知。               | [房间属性](https://doc.commsease.com/neroom/guide/jUxMDE5OTM?platform=android)       |
| 成员属性设置和更新             | 成员属性是附加到房间特定成员上的一系列 key-value键值对， key为属性唯一名称， value为当前属性值。 NERoom允许开发者自定义成员属性，并在房间生命周期内对成员属性进行添加、更新、删除操作，同时还可以监听属性变更通知。                    | [成员属性](https://doc.commsease.com/neroom/guide/jkyNTk0Njc?platform=android)       |
| 静音和取消静音房间内其他成员   | 拥有相应权限的角色，可以静音或取消静音房间内某个成员。      | [静音和取消静音](https://doc.commsease.com/neroom/guide/Dg5NDM1NDY?platform=android) |
| 开启和关闭房间内其他成员的视频 | 拥有相应权限的角色，可以开启和关闭房间内某个成员的视频。           | [开启和关闭视频](https://doc.commsease.com/neroom/guide/jk4MTc3NjQ?platform=android) |
| 设置本地视图                   | 在视频通话前，可以设置本地视图。            | [本地视频预览](https://doc.commsease.com/neroom/guide/jQzODE5NjI?platform=android)   |
| 关闭某成员的屏幕共享           | 拥有相应权限的角色，可以关闭某个成员的屏幕共享。     | [屏幕共享](https://doc.commsease.com/neroom/guide/DEwODM5MDg?platform=android)       |
| 关闭某成员的白板共享           | 拥有相应权限的角色，可以关闭某个成员的白板共享。         | [互动白板](https://doc.commsease.com/neroom/guide/TM4NDk2NjQ?platform=android)       |
| 直播                           | NERoom 的直播基于专业的跨平台视频编解码技术和大规模视频内容分发网络，提供稳定流畅、高可靠、高并发的直播服务，助力轻松打造企业级在线直播平台。      | [直播](https://doc.commsease.com/neroom/guide/zA0ODQ1NDk?platform=android)           |
