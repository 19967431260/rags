...
<video src="https://yx-web-nosdn.netease.im/common/990c0bfc25487aa4e94840678b2f482b/实现音视频安全检测.mp4" poster="https://yx-web-nosdn.netease.im/common/a307a78d9b5c43be657b6a8f68408ffb/网易云信小课堂.png" controls="controls" preload="auto" width="720" height="405"></video>
...
- 登录云信控制台创建应用、申请开通**音视频通话 2.0 **服务和**安全通**服务。具体步骤请参见[音视频通话2.0接入流程](https://doc.yunxin.163.com/nertc/server-apis/DkyMDM2Mzk?platformId=50002)和[开通安全通服务](https://doc.yunxin.163.com/nertc/server-apis/zMzMTYzNDM?platformId=50326)。
- 实现基础的音视频通话功能，具体操作步骤请参见[实现音视频通话](https://doc.yunxin.163.com/nertc/server-apis/DQ4MDk2NTA?platformId=50002)。
- 准备接收抄送消息的服务器公网地址，并配置消息抄送。配置方法请参见[开通消息抄送](https://doc.yunxin.163.com/nertc/server-apis/DExNjg2MDc?platformId=50326)。审核结果会以消息抄送发送至您预留的抄送地址。
- 配置内容检测相关规则。在**安全通**功能页面，单击右上角的**更多安全配置**，配置安全检测策略。具体步骤请参见[开通安全通服务](https://doc.yunxin.163.com/nertc/server-apis/zMzMTYzNDM?platformId=50326)。
- 准备音视频通话开发环境。

### 步骤二 创建音视频通话

1. （可选）[创建房间](https://doc.yunxin.163.com/nertc/server-apis/jg3NjcyNTE?platformId=50326)，传入指定的房间名及用户ID。
...
为对应房间开启安全通审核任务，具体步骤请参见[创建安全通审核任务](https://doc.yunxin.163.com/nertc/server-apis/DA3OTIwNzg?platformId=50326)。
<br>您可根据自身业务选择是否开启自动打码，详细信息请参见[安全通审核自动打码](https://doc.yunxin.163.com/nertc/server-apis/DI5MDE5NTE)。

示例代码如下：
```
curl --location --request POST 'https://rtc.yunxinapi.com/livewallsolution/submit' \
--header 'AppKey: ***' \  //请使用您应用的appkey
--header 'Nonce: ***' \  //随机数，最大长度为 128 个字符。
--header 'CurTime: 1639555317' \  //当前 Unix 时间戳，即从 1970 年 1 月 1 日 0 点 0 分 0 秒开始到现在的秒数。数据类型为 String。
--header 'CheckSum: ba193e4d4ba3991987eb72afe692095598552b79' \ //计算方式详见：https://doc.yunxin.163.com/nertc/server-apis/TA5MjEzNTM?platformId=50326
...
当音视频内容没有违规内容时，不会触发审核抄送。只有在音视频内容有违规问题时，才会触发具体异常详情抄送，具体请参见[安全通审核事件](https://doc.yunxin.163.com/nertc/server-apis/jYyMzk5Nzc?platformId=50326)。
...
[安全通审核事件字段说明](https://doc.yunxin.163.com/nertc/server-apis/jYyMzk5Nzc?platformId=50326#字段说明)。
    ```
    {
	"data": {
...
以下示例代码以移除违规房间成员为例，服务端移除成员的接口说明请参见[移除成员](https://doc.yunxin.163.com/nertc/server-apis/TA5MTExNjg?platformId=50326)。


```
 curl --location --request POST 'https://rtc.yunxinapi.com/v3/api/kicklist/members?cname=neteasetest1&uid=12345' \
        --header 'AppKey: ***' \  //请使用您应用的appkey
        --header 'Nonce: ***' \  //随机数，最大长度为 128 个字符。
        --header 'CurTime: 1639555317' \  //当前 Unix 时间戳，即从 1970 年 1 月 1 日 0 点 0 分 0 秒开始到现在的秒数。数据类型为 String。
        --header 'CheckSum: ba193e4d4ba3991987eb72afe692095598552b79' \ //计算方式详见：https://doc.yunxin.163.com/nertc/server-apis/TA5MjEzNTM?platformId=50326
...
[停止安全通审核任务](https://doc.yunxin.163.com/nertc/server-apis/Dg5NzQ1MjM?platformId=50326)。

示例代码如下：

```
curl --location --request POST 'https://rtc.yunxinapi.com/livewallsolution/feedback' \
--header 'AppKey: ***' \  //请使用您应用的appkey
--header 'Nonce: ***' \  //随机数，最大长度为 128 个字符。
--header 'CurTime: 1639555317' \  //当前 Unix 时间戳，即从 1970 年 1 月 1 日 0 点 0 分 0 秒开始到现在的秒数。数据类型为 String。
--header 'CheckSum: ba193e4d4ba3991987eb72afe692095598552b79' \ //计算方式详见：https://doc.yunxin.163.com/nertc/server-apis/TA5MjEzNTM?platformId=50326
