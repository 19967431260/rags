...
- 下载 c# SDK，如果安装了 git 命令行，执行 `git clone https://github.com/vcloud163/csharp-sdk.git`。
- 参照 API 说明和 SDK 中提供的 demo，开发代码。

### <span id="HTTPS 支持">HTTPS 支持</span>

默认使用 https 协议，如需修改为 http 协议，请将 com.netease.vcloud.util 包中的 UploadUtil 类的具体方法中的 url 修改。
...
视频云点播在全国各地覆盖大量上传节点，会选择适合用户的最优节点进行文件上传，并根据用户传入的参数做不同处理，具体请参考点播服务端 API 文档。
...
:::note note
具体使用示例请参考 SDK com.netease.vcloud.upload.demo 包中 QueryOffsetDemo。
:::
...
:::note note
具体使用示例请参考 SDK com.netease.vcloud.upload.demo 包中 Upload_RecoderDemo。
:::
...
:::note note
具体使用示例请参考 SDK com.netease.vcloud.upload.demo 包中 QueryVideoIDDemo。
:::
...
#上传成功后回调客户端的 URL 地址（需标准 http 格式）
...
:::note note
具体使用示例请参考 SDK com.netease.vcloud.upload.demo 包中 SetCallbackDemo。
:::
...
CSharp SDK 的初始版本，提供点播上传的基本功能。包括：文件上传、获取进度、断点续传、查询视频、设置回调。