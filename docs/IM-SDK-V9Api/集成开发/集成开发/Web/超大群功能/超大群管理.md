...
<!-- keywords: IM超大群,超大群集成,超大群功能 -->
<!-- description: 网易云信IM即时通讯超大群功能说明,超大群客户端开发指南-->
...
这里的参数并不是所有的初始化参数, 可以访问[初始化SDK API](https://dev.yunxin.163.com/docs/interface/即时通讯Web端/NIMSDK-Web/NIM.html#NIM__anchor)查阅所有的初始化参数。
...
- `teamId`: 群ID
- `account`: 帐号
- `type`: 超大群群成员类型，`'owner'` 群主、`'normal'` 普通成员和 `'manager'` 管理员
- `nickInTeam`: 在群里面的昵称
- `active`: 激活状态
- `joinTime`: 入群时间
- `updateTime`: 更新时间
...
发送超大群群消息时只需将各个[发送消息](/docs/TM5MzM5Njk/jg0NTA4NjE#发送消息)接口的`scene`替换为`'superTeam'`, 将`to`替换为`超大群ID`即可。

## <span id="接收超大群群消息">接收超大群群消息</span>

参考[接收消息](/docs/TM5MzM5Njk/jg0NTA4NjE#接收消息)
...
[拉人入群](/docs/TM5MzM5Njk/DY2NjIzODc#拉人入群)后, 所有[群成员](/docs/TM5MzM5Njk/DY2NjIzODc#超大群群成员对象)会收到一条类型为`'addSuperTeamMembers'`的[群通知消息](/docs/TM5MzM5Njk/jg0NTA4NjE#超大群群通知消息)。此类群通知消息的`from`字段的值为拉人的人的帐号, `to`字段的值为对应的群ID, `attach`有一个字段`team`的值为对应的[超大群对象](/docs/TM5MzM5Njk/DY2NjIzODc#超大群对象), `attach`有一个字段`accounts`的值为被拉的人的帐号列表, `attach`有一个字段`members`的值为被拉的群成员信息列表。
...
- [踢人出群](/docs/TM5MzM5Njk/DY2NjIzODc#踢人出群)后, 所有[群成员](/docs/TM5MzM5Njk/DY2NjIzODc#超大群群成员对象)会收到一条类型为`'removeTeamMembers'`的[群通知消息](/docs/TM5MzM5Njk/jg0NTA4NjE#超大群群通知消息)。此类群通知消息的`from`字段的值为踢人的人的帐号, `to`字段的值为对应的群ID, `attach`有一个字段`team`的值为对应的[超大群对象](/docs/TM5MzM5Njk/DY2NjIzODc#超大群对象), `attach`有一个字段`accounts`的值为被踢的人的帐号列表。
...
- [主动退群](/docs/TM5MzM5Njk/DY2NjIzODc#主动退群)后, 所有[超大群群成员](/docs/TM5MzM5Njk/DY2NjIzODc#超大群群成员对象)会收到一条类型为`'leaveSuperTeam'`的[群通知消息](/docs/TM5MzM5Njk/jg0NTA4NjE#超大群群通知消息)。此类群通知消息的`from`字段的值为退群的人的帐号, `to`字段的值为对应的群ID, `attach`有一个字段`team`的值为对应的[超大群对象](/docs/TM5MzM5Njk/DY2NjIzODc#超大群对象)。
...
目前支持修改的属性有这些
- `nickInTeam`: 自己在群里面的群昵称。更新昵称后, 所有其它在线的[超大群群成员](/docs/TM5MzM5Njk/DY2NjIzODc#超大群群成员对象)会收到[初始化SDK](/docs/TM5MzM5Njk/zE0NDY4Njc)时传入的`onUpdateSuperTeamMember`回调
...
- 如果开发者在[初始化SDK](/docs/TM5MzM5Njk/zE0NDY4Njc)的时候设置了`syncSuperTeams`为`false`, 那么就收不到`onSuperTeams`回调, 可以调用此方法来获取超大群列表
...
- [通过入群申请](https://dev.yunxin.163.com/docs/interface/即时通讯Web端/NIMSDK-Web/NIM.html#passSuperTeamApply__anchor)
- [拒绝入群申请](https://dev.yunxin.163.com/docs/interface/即时通讯Web端/NIMSDK-Web/NIM.html#rejectSuperTeamApply__anchor)
- [添加群成员/邀请入群](https://dev.yunxin.163.com/docs/interface/即时通讯Web端/NIMSDK-Web/NIM.html#addSuperTeamMembers__anchor)
- [接受入群邀请](https://dev.yunxin.163.com/docs/interface/即时通讯Web端/NIMSDK-Web/NIM.html#acceptSuperTeamInvite__anchor)
- [拒绝入群邀请](https://dev.yunxin.163.com/docs/interface/即时通讯Web端/NIMSDK-Web/NIM.html#rejectSuperTeamInvite__anchor)
- [添加群管理员](https://dev.yunxin.163.com/docs/interface/即时通讯Web端/NIMSDK-Web/NIM.html#addSuperTeamManagers__anchor)
- [移除群管理员](https://dev.yunxin.163.com/docs/interface/即时通讯Web端/NIMSDK-Web/NIM.html#removeSuperTeamManagers__anchor)
- [更新群成员的禁言状态](https://dev.yunxin.163.com/docs/interface/即时通讯Web端/NIMSDK-Web/NIM.html#updateSuperTeamMembersMute__anchor)
- [更新超大群的禁言状态](https://dev.yunxin.163.com/docs/interface/即时通讯Web端/NIMSDK-Web/NIM.html#updateSuperTeamMute__anchor)
- [修改别人的群昵称](https://dev.yunxin.163.com/docs/interface/即时通讯Web端/NIMSDK-Web/NIM.html#updateNickInSuperTeam__anchor)
