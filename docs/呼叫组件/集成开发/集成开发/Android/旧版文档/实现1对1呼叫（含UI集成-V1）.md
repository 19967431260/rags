...
呼叫组件（NERTCCallkit）通过 UI 组件化的方式，简化了呼叫流程，您只需要调用几行代码，就可以实现 1 对 1 呼叫，并包含呼叫的 UI 界面。本文介绍呼叫组件 **V1 版本**的集成和实现方法。

::: note note
推荐使用含 UI 集成呼叫组件 V2 版本，请参见[实现1对1呼叫（含UI集成-V2）](https://doc.yunxin.163.com/nertccallkit/docs/jcyNDE3MDM?platform=android)。
:::
...
- 呼叫组件（NERTCCallkit）默认使用 RTC 调试模式。应用正式上线前，请在控制台中将指定应用的 RTC 鉴权方式改回安全模式。安全模式下需要使用 Token 加入房间，详细说明请参考[Token 鉴权](/docs/jcyOTA0ODM/TQ0MTI2ODQ)。
...
- 已在控制台创建应用，并获取了对应的 App Key。详细操作请参考<a href="https://doc.yunxin.163.com/console/docs/TIzMDE4NTA?platform=console" target="_blank">创建应用</a>。

- 已开通 IM 即时通讯、音视频通话 2.0 及信令产品。开通服务的操作流程请参考<a href="https://doc.yunxin.163.com/nertccallkit/docs/zM0ODQ4NDI?platform=android" target="_blank">开通服务</a>。

- 已在网易云信控制台开通音视频通话 2.0 产品的话单功能。详细操作请参考 <a href="/docs/zMwMzkwNTE/TkyODAzMDQ#1%20开通话单功能" target="_blank">话单</a>。
...
- [自定义UI](/docs/zMwMzkwNTE/jE1NDM1ODI)
- [话单](/docs/zMwMzkwNTE/TkyODAzMDQ)
- [设置 RTC 的音视频属性](https://doc.yunxin.163.com/nertccallkit/docs/DA3ODA0MDQ?platform=android)
- [自定义呼叫铃声](https://doc.yunxin.163.com/nertccallkit/docs/zkyMTI3ODA?platform=android)
- [拦截呼入请求](https://doc.yunxin.163.com/nertccallkit/docs/zY2MDY2MDI?platform=android)
